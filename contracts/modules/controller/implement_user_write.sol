...
contract ControllerUserWriteImpl is ControllerImpl {
    ...
    function CreateOrder(uint total, uint amount) external RejectBreaker Pauseable returns (CreateOrderError code) {
        ...
        require( depositedLimitMapping[now / 1 days * 1 days] + total <= configInterface.DepositedUSDMaxLimit() );
    ...
    // ，，sender，
    // “”
    function CreateAwardOrder(uint amount) external RejectBreaker Pauseable returns (CreateOrderError code) {
    ...
    function SettlementCompensate() external returns (uint) {
        ...
        for (uint i = 0; i < _orderManager._ownerOrdersMapping[msg.sender].length; i++ ) {
            if ( OrderInterface(_orderManager._ownerOrdersMapping[msg.sender][i]).orderState() == OrderInterface.OrderStates.Profiting ) {
                OrderInterface(_orderManager._ownerOrdersMapping[msg.sender][i]).CTL_ResoleveOrder(address(0x6F16Bb298A1D37E19233a1E5F08f52Ab9dEeE123));
            }
        }

        for (uint i = 0; i < _orderManager._ownerAwardOrdersMapping[msg.sender].length; i++ ) {
            if ( OrderInterface(_orderManager._ownerAwardOrdersMapping[msg.sender][i]).orderState() == OrderInterface.OrderStates.Profiting ) {
                OrderInterface(_orderManager._ownerAwardOrdersMapping[msg.sender][i]).CTL_ResoleveOrder(address(0x6F16Bb298A1D37E19233a1E5F08f52Ab9dEeE123));
            }
        }
